 ...
 * @route GET /api/admin/dashboard (for seller)
 * @access Private
 */
...
exports.getBuyerDashboard = async (req, res, next) => {
  try {
    const userId = req.user._id;
    const purchasedItems = await Product.find({ buyer: userId, isSold: true }).populate(
      "seller",
      "name email"
    );
    const activeInterests = await Product.find({ intentBy: userId, isSold: false }).populate(
      "seller",
      "name email"
    );
...
exports.deleteCategory = async (req, res, next) => {
  try {
    ...
    if (productsCount > 0) {
      return next(
        createError(400, `Cannot delete category. It is being used by ${productsCount} products.`)
      );
    }
...
exports.updateSettings = async (req, res, next) => {
  try {
    ...
    settings.featuredProductsLimit =
      featuredProductsLimit || settings.featuredProductsLimit;
